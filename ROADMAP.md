...
- [X] **Ordered handlers and behaviors** — optional `ServiceOrderAttribute` on handlers and pipeline behaviors to control execution order when multiple are registered for the same message type. By default, handlers execute in registration order and manually registered behaviors execute with the first registered behavior outermost, but use ordinary sorting rules when the source generator is used since it generates a single static pipeline per message type.
