...
public interface IMediatorServiceBuilder
{
    ...
    // Keyed handlers are addressed by a service key at dispatch time, enabling
    // runtime routing strategies. Pipeline behaviors are NOT applied to keyed dispatch.

    /// <summary>
    /// Registers a command handler under a service key for keyed routing.
    /// </summary>
    /// <remarks>
    /// Keyed handlers bypass the pre-compiled pipeline. Use <c>IMediator.Send(key, message)</c>
    /// to dispatch to handlers registered under a specific key.
    /// </remarks>
    IMediatorServiceBuilder RegisterKeyedCommandHandler<
        [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] THandler,
        TMessage>(object? key)
        where THandler : class, ICommandHandler<TMessage>
        where TMessage : notnull;
    ...
    /// <summary>
    /// Registers a notification handler under a service key for keyed routing.
    /// </summary>
    /// <remarks>
    /// Keyed handlers bypass the pre-compiled pipeline. Use <c>IMediator.Notify(key, message)</c>
    /// to dispatch to handlers registered under a specific key.
    /// </remarks>
    IMediatorServiceBuilder RegisterKeyedNotificationHandler<
        [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] THandler,
        TMessage>(object? key)
        where THandler : class, INotificationHandler<TMessage>
        where TMessage : notnull;
    ...
    /// <summary>
    /// Registers a request handler under a service key for keyed routing.
    /// </summary>
    /// <remarks>
    /// Keyed handlers bypass the pre-compiled pipeline. Use <c>IMediator.Request(key, message)</c>
    /// to dispatch to the handler registered under a specific key.
    /// </remarks>
    IMediatorServiceBuilder RegisterKeyedRequestHandler<
        [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] THandler,
        TMessage,
    ...
    /// <summary>
    /// Registers a stream handler under a service key for keyed routing.
    /// </summary>
    /// <remarks>
    /// Keyed handlers bypass the pre-compiled pipeline. Use <c>IMediator.RequestStream(key, message)</c>
    /// to dispatch to handlers registered under a specific key.
    /// </remarks>
    IMediatorServiceBuilder RegisterKeyedStreamHandler<
        [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] THandler,
        TMessage,
        TResponse>(object? key)
        where THandler : class, IStreamHandler<TMessage, TResponse>
